import React, { useState } from "react";
import { Search, ChevronUp, ChevronDown } from "lucide-react";
import { AnimatePresence, motion } from "framer-motion";

/* =======================
   Types
======================= */

export interface VenuesFilterValue {
  search?: string;
  ordering?: string;
}

interface VenuesFilterProps {
  value: VenuesFilterValue;
  onChange: (value: VenuesFilterValue) => void;
}

/* =======================
   Component
======================= */

export const VenuesFilter: React.FC<VenuesFilterProps> = ({
  value,
  onChange,
}) => {
  const [dropdownOpen, setDropdownOpen] = useState(false);

  const sortOptions: {
    label: string;
    value:
      | "name_asc"
      | "name_desc"
      | "price_low"
      | "price_high"
      | "newest"
      | "oldest";
  }[] = [
    { label: "Name (A–Z)", value: "name_asc" },
    { label: "Name (Z–A)", value: "name_desc" },
    { label: "Price (Low → High)", value: "price_low" },
    { label: "Price (High → Low)", value: "price_high" },
    { label: "Newest", value: "newest" },
    { label: "Oldest", value: "oldest" },
  ];

  const sortToOrderingMap: Record<(typeof sortOptions)[number]["value"], string> = {
    name_asc: "name",
    name_desc: "-name",
    price_low: "price_per_hour",
    price_high: "-price_per_hour",
    newest: "-created_at",
    oldest: "created_at",
  };

  const currentSortKey =
    Object.entries(sortToOrderingMap).find(
      ([, v]) => v === value.ordering
    )?.[0] ?? null;

  return (
    <motion.div
      className="mb-6 rounded-2xl border border-gray-100 bg-white p-4 font-nata-sans-rg"
      initial={{ opacity: 0, y: 8 }}
      animate={{ opacity: 1, y: 0 }}
    >
      <div className="flex flex-col gap-4 md:flex-row">
        {/* ---------- Search ---------- */}
        <div className="relative flex-1">
          <Search
            size={18}
            className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400"
          />
          <input
            type="text"
            placeholder="Search venues..."
            value={value.search ?? ""}
            onChange={(e) =>
              onChange({
                ...value,
                search: e.target.value || undefined,
              })
            }
            className="w-full rounded-xl border py-2 pl-10 pr-4 outline-none hover:bg-emerald-50/20 focus:ring-1 focus:ring-emerald-200"
          />
        </div>

        {/* ---------- Ordering ---------- */}
        <div className="relative w-56">
          <button
            onClick={() => setDropdownOpen((prev) => !prev)}
            className="flex w-full items-center justify-between rounded-xl
                       border px-4 py-2 text-sm text-gray-400 shadow-sm
                       outline-none transition hover:bg-emerald-50/20 
                       focus:ring-1 focus:ring-emerald-200"
          >
            {sortOptions.find((o) => o.value === currentSortKey)?.label ??
              "Sort by"}

            {dropdownOpen ? (
              <ChevronUp className="h-4 w-4 text-gray-500" />
            ) : (
              <ChevronDown className="h-4 w-4 text-gray-500" />
            )}
          </button>

          <AnimatePresence>
            {dropdownOpen && (
              <motion.div
                initial={{ opacity: 0, y: -5 }}
                animate={{ opacity: 1, y: 0 }}
                exit={{ opacity: 0, y: -5 }}
                transition={{ duration: 0.2 }}
                className="absolute z-50 mt-1 w-full overflow-hidden
                   rounded-xl border border-emerald-300
                   bg-white shadow-lg"
              >
                {sortOptions.map((option) => (
                  <button
                    key={option.value}
                    onClick={() => {
                      onChange({
                        ...value,
                        ordering: sortToOrderingMap[option.value],
                      });
                      setDropdownOpen(false);
                    }}
                    className={`w-full px-4 py-2 text-left text-sm transition
                        hover:bg-emerald-500/10
                        ${
                          currentSortKey === option.value ? "font-semibold" : ""
                        }`}
                  >
                    {option.label}
                  </button>
                ))}
              </motion.div>
            )}
          </AnimatePresence>
        </div>
      </div>
    </motion.div>
  );
};
